...
int fmc_imageon_hdmio_init( fmc_imageon_t *pContext, Xuint32 Enable, fmc_imageon_video_timing_t *pTiming, Xuint32 WaitForHPD )
{
   ...
   else
   {
	  ...
	  // Adjust HDMI/DVI mode based on specified video timing info
	  {
		 ...
		 for ( i = 0; i < IIC_HDMI_OUT_CONFIG_LEN; i++ )
		 {
		    if ( iic_hdmi_out_config[i][1] == 0xAF )
			{
				if ( pTiming->IsHDMI )
				{
                   if ( pContext->bVerbose ) uart_puts( "\tSetting I2C config sequence for HDMI Mode\n\r" );
                   iic_hdmi_out_config[i][2] = 0x06; // Set HDMI Mode
                                                     //    R0xAF[  1] = HDMI/DVI Mode Select = 1 (HDMI Mode)
				}
				else
				{
                   if ( pContext->bVerbose ) uart_puts( "\tSetting I2C config sequence for DVI Mode\n\r" );
      ...
      if (num_bytes < 1)
      {
         uart_puts("\t\tERROR occurred during I2C transaction !\n\r");
      }
      ...
      if ( WaitForHPD )
      {
         ...
         while (((i2c_data & 0xC0) != 0xC0) && (--timeout))
         {
            ...
            if (num_bytes < 1)
            {
               uart_puts( "[fmc_imageon_hdmio_init] ERROR occurred during I2C transaction !\n\r");
               timeout = 0;
               return 0;
            }
            //printf("\t0x%02X[0x%02X] => 0x%02X\n\r", (i2c_dev << 1), i2c_addr, i2c_data);
         ...
         if (!timeout)
         {
            uart_puts( "[fmc_imageon_hdmio_init] ERROR - Timeout waiting for internal HotPlugDetect !\n\r");
            return 0;
         }
      } // if ( WaitForHPD )
//uart_puts("fmc_imageon_hdmio_init end!\n");
   }
