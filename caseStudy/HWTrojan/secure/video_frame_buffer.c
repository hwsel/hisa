...
int vfb_common_init( u16 uDeviceId, XAxiVdma *pAxiVdma )
{
   ...
   {
      printf( "vfb_common_init: Initialization failed %d\n\r", Status);
      return 1;
   }
...
int vfb_rx_setup(XAxiVdma *pAxiVdma, XAxiVdma_DmaSetup *pWriteCfg, Xuint32 uVideoResolution, Xuint32 uStorageResolution, Xuint32 uMemAddr, Xuint32 uNumFrames )
{
	...
	video_width  = vres_get_width ( uVideoResolution ) * 4; // in bytes

	// Get Storage dimensions
	storage_height = vres_get_height( uStorageResolution );      // in lines
	storage_width  = vres_get_width ( uStorageResolution ) * 4; // in bytes
	storage_stride = storage_width;
	storage_size   = storage_width * storage_height;
//	storage_offset = ((storage_height-video_height)>>2)*storage_width + ((storage_width-video_width)/4);
...
int vfb_tx_start(XAxiVdma *pAxiVdma)
{
   ...
   if (Status != XST_SUCCESS)
   {
      printf("vfb_tx_start: Start read transfer failed %d\n\r", Status);
      return XST_FAILURE;
   }
...
int vfb_check_errors(XAxiVdma *pAxiVdma, u8 bClearErrors )
{
   ...
   uart_puts( "AXI_VDMA - Checking Error Flags\n\r" );
   ...
   if ( Errors )
   {
	   ...
	   {
          uart_puts( "\tS2MM_DMASR - ErrIrq\n\r" );
	   }
	   if ( inErrors & 0x00008000 )
	   {
          uart_puts( "\tS2MM_DMASR - EOLLateErr\n\r" );
	   }
	   if ( inErrors & 0x00000800 )
	   {
          uart_puts( "\tS2MM_DMASR - SOFLateErr\n\r" );
	   }
	   if ( inErrors & 0x00000400 )
	   {
          uart_puts                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    ( "\tS2MM_DMASR - SGDecErr\n\r" );
	   }
	   if ( inErrors & 0x00000200 )
	   {
          uart_puts( "\tS2MM_DMASR - SGSlvErr\n\r" );
	   }
	   if ( inErrors & 0x00000100 )
	   {
         uart_puts( "\tS2MM_DMASR - EOLEarlyErr\n\r" );
	   }
	   if ( inErrors & 0x00000080 )
	   {
          uart_puts( "\tS2MM_DMASR - SOFEarlyErr\n\r" );
	   }
	   if ( inErrors & 0x00000040 )
	   {
          uart_puts( "\tS2MM_DMASR - DMADecErr\n\r" );
	   }
	   if ( inErrors & 0x00000020 )
	   {
          uart_puts( "\tS2MM_DMASR - DMASlvErr\n\r" );
	   }
	   if ( inErrors & 0x00000010 )
	   {
          uart_puts( "\tS2MM_DMASR - DMAIntErr\n\r" );
	   }

	   if ( outErrors & 0x00004000 )
	   {
          uart_puts( "\tMM2S_DMASR - ErrIrq\n\r" );
	   }
	   if ( outErrors & 0x00000400 )
	   {
          uart_puts( "\tMM2S_DMASR - SGDecErr\n\r" );
	   }
	   if ( outErrors & 0x00000200 )
	   {
          uart_puts( "\tMM2S_DMASR - SGSlvErr\n\r" );
	   }
	   if ( outErrors & 0x00000080 )
	   {
          uart_puts( "\tMM2S_DMASR - SOFEarlyErr\n\r" );
	   }
	   if ( outErrors & 0x00000040 )
	   {
          uart_puts( "\tMM2S_DMASR - DMADecErr\n\r" );
	   }
	   if ( outErrors & 0x00000020 )
	   {
          uart_puts( "\tMM2S_DMASR - DMASlvErr\n\r" );
	   }
	   if ( outErrors & 0x00000010 )
	   {
          uart_puts( "\tMM2S_DMASR - DMAIntErr\n\r" );
	   }

	   // Clear error flags
	   uart_puts( "AXI_VDMA - Clearing Error Flags\n\r" );
