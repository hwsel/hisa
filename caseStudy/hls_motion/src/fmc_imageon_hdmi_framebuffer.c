...
//               *filter = 0x40; filter=filter+1;
   ...
   xil_printf( "Video DMA (Output Side) Initialization new...\n\r" );
   vfb_common_init(
...
//   xil_printf( "Video DMA (Input Side) Initialization ...\n\r" );
   ...
   if(iterations ==10)memcpy(FirstFrame, pStorageMem, storage_size/*new_storage_size*/);
   //>>>>>>>>>>>>>>>>>>>>>add this <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
